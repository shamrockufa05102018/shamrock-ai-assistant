...
function recordResult(result, amount) {
  ...
  const messages = {
    offered: "Записано: сотрудник предложил позицию.",
    bought: `Записано: гость купил, допродажа ${formatRub(amount)}.`,
    rejected: "Записано: гость отказался.",
    not_offered: "Записано: сотрудник не предложил. Это видно владельцу.",
  };
...
document.addEventListener("click", (event) => {
  ...
  recordResult(button.dataset.result, Number(button.dataset.amount || 0));
});
